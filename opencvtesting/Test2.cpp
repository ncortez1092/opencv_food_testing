...
Mat result,d1,d2,motionImg,current,next;
Mat old;
RNG rng(12345);
void motionSensor();
...
while(true)
{
    ...
    cout << "Frame size : " << LWidth << " x " << LHeight << endl; // Mainly used to see how often we are getting an image, but useful info
        ...
        vector<vector<Point> > contours; // Used for contours, motion detecting
        vector<Vec4i> heir;// Same as contours
        ...
        resize(Pano, Pano, Size(640,480),0,0,INTER_CUBIC); // Resize our pano image so its a bit smaller
        ...
        absdiff(old,current,d1); 
        threshold(d1,d1,35,255,CV_THRESH_BINARY);
        findContours(d1,contours, heir,CV_RETR_TREE,CV_CHAIN_APPROX_SIMPLE);
        /*
        Mat drawing = Mat::zeros(d1.size(),CV_8UC3);
        for (int i = 0; i < contours.size(); i++)
        {
            Scalar color = Scalar (rng.uniform(0,255),rng.uniform(0,255), rng.uniform(0,255));
            drawContours(drawing, contours, i, color, 2, 8, heir, 0, Point());
        }
*/
